...
def lambda_handler(event, context):
    ...
    After that, this function handles records deletion sql from given table names 
    
    Finally this function uses sql insertion querry to insert records from data set which is uploaded to 'disease-park-data-xlsx-aka-disease-table' s3 bucket
    ...
    if event:
        ...
        table_list = ['SEASON_ENT_DATA', 'DISEASE_SEASON', 'DISEASE_TIP',
                    'DISEASE_SYMPTOM', 'SEASON', 'AREA', 'TIP', 'SYMPTOM', 'DISEASE', 'LGA', 'LOCALITY']
        
        # loop to delete all the records
